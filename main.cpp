...
#include "OPENGL/src/vendor/glm/glm.hpp" // bibliothèque de maths adaptée à OpenGL
#include "OPENGL/src/vendor/glm/gtc/matrix_transform.hpp"// bibliothèque de maths adaptée à OpenGL

#include "OPENGL/src/vendor/imgui/imgui.h" // ImGui
#include "OPENGL/src/vendor/imgui/imgui_impl_glfw_gl3.h"// ImGui

// Les fichiers ci-dessous sont ceux que nous avons fait, avec l'aide du tutoriel OpenGl de The cherno pour les fichiers du dossier OPENGL, et inspiration du travail de Sebastian Lague pour débuter la simulation du fluide
...
int main(){
    ...
    bool initial = true;// True tant que la définition des paramètres n'est pas finie
    ...
    bool affiche_vitesses_colorees = false; // permet de créer un gradient de couleur entre les particules lentes et rapides. 
    ...
    // variables  servant à interagir avec le fluide en imposant des forces extérieures. Le clique gauche n'est pas consistent, il est recommendé d'utiliser "a" et "e"
    float sourisx = 0; // position du curseur
    float sourisy = 0;
    bool clique_gauche=false; //Variables contenant true si la touche est pressée.
    ...
    // Paramètres partagés, modifiables durant la simulation
    ...
    float rayon_influence =0.1; // Partagé pour simplifier les interactions entre particules différentes et simplifier lors de l'utilisation des listes d'indices
    ...
    int nb =nombre_de_particules; // Pour l'initialisation car je n'ai pas trouvé d'équivalent à ImGui::InputInt pour un unsigned int.

    // Paramètres modifiables durant la simulation
    float rayon_collision = 0.01; // rayon utilisé pour les collisions avec les parois et les objets. N'intervient pas dans les interactions particulaires. Rayon utilisé pour l'affichage (sans flou)
    float g = 10; // contrôle la norme de la gravité. À utiliser pour faire varier les densités des fluides (cf rapport )
    float masse = 1; // Proportionnel à la masse ajoutée par particule, répartie dans le disque de rayon rayon_influence. 
    float multiplicateur_pression = 100; // Force de l'interaction de répulsion/attraction entre 2 particules de même type
    float multiplicateur_pression_proche = 1000; // Force de l'interaction de répulsion entre 2 particules de même type à courte distance. Aide à la génération de pseudo-tensions de surface
    float densite_visee = 1000; // Densité à laquelle il n'y a plus de forces de pression. (il peut encore y avoir de la force de pression proche)
    ...
    int texture_debut = 0; // utilisation de texture pour le fluide, celles qui sont bind entre texturedebut et texturefin. 0 correspond à aucune texture.
    int texture_fin = 0;

    // Paramètres pour le second fluide; il sera défini si nombre_de_particule2>0

    unsigned int nombre_de_particules2 =0;// Non modifiable durant la simulation
    int nb2 =nombre_de_particules2; // Pour l'initialisation car je n'ai pas trouvé d'équivalent à ImGui::InputInt pour un unsigned int.
    
    
    // Paramètres modifiables durant la simulation
    float rayon_collision2 = 0.01; // rayon utilisé pour les collisions avec les parois et les objets. N'intervient pas dans les interactions particulaires. Rayon utilisé pour l'affichage (sans flou)
    float g2 = 10; // contrôle la norme de la gravité. À utiliser pour faire varier les densités des fluides ( cf rapport )
    float masse2 = 1; // Proportionnel à la masse ajoutée par particule, répartie dans le disque de rayon rayon_influence. 
    float multiplicateur_pression2 = 100; // Force de l'interaction de répulsion/attraction entre 2 particules de même type
    float multiplicateur_pression_proche2 = 1000; // Force de l'interaction de répulsion entre 2 particules de même type à courte distance. Aide à la génération de pseudo-tensions de surface
    float densite_visee2 = 1000; // Densité à laquelle il n'y a plus de force de pression. (il peut encore y avoir de la force de pression proche)
    ...
    int texture_debut2 = 0; // utilisation de texture pour le fluide, celles qui sont bind entre texturedebut et texturefin. 0 correspond à aucune texture.
    int texture_fin2 = 0;
    ...
    // Active la possibilité de transparence et définit la fonction de transparence
    ...
    // matrice de transformation, mais peu utilisée dans notre cas
    ...
    // Initialisation des objets nécessaires pour l'affichage
    ...
    while (initial && (!glfwWindowShouldClose(window)))
    {
        ...
        // actualise les constantes à l'aide d'ImGui
 ...
 &coeff_amorti2, &coeff_viscosite2, &coeff_adherence2, &logg2, &logmp2, &logmpp2, &logvisc2,&texture_debut2,&texture_fin2);
        ...
        //Fenêtre ImGui permettant d'initialiser les nombres de particules et de valider
     ...
     // Initialisation des deux liquides
    ...
    //Tableau pour tracer tous les points en un seul "draw"
 ...
 &coeff_amorti2, &coeff_viscosite2, &coeff_adherence2, &logg2, &logmp2, &logmpp2, &logvisc2,&texture_debut2,&texture_fin2);
        ...
        // calcul et affichage de la densité si l'option est activée
        if (affiche_densite){
        ...
        // remplit les informations pour l'affichage des particules, par point : xpoint,ypoint,xparticule,yparticule,R,G,B,A,TexCoordx,TexCoordy,TexID,rayon_collision²,rayon_influence²
